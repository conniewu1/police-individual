...
\begin{center}
$$
log(\frac{p_i}{(1-p_i)}) = \beta_0 + \beta_1\ muEDA_i + \beta_2\ sdEDA_i + \beta_3\ muEDA_i:sdEDA_i + \beta_4\ muTemp_i + \beta_5\ sdTemp_i + \beta_6\ muEMG_i
$$
$$
+ \beta_7\ sdEMG_i + \beta_8\ muACC\_Y_i + \beta_9\ sdACC\_Y_i + \beta_{10}\ muACC\_Y_i:muEDA_i + \beta_{11}\ muEDA_i:muTemp_i
$$
\end{center}
|        The final model for multiple-victim shootings written out below:
\begin{center}
$$
log(\frac{p_i}{(1-p_i)}) = \beta_0 + \beta_1\ muEDA_i + \beta_2\ sdEDA_i + \beta_3\ muEDA_i:sdEDA_i + \beta_4\ muTemp_i + \beta_5\ sdTemp_i + \beta_6\ muEMG_i
$$
...
|        For sensitivity analysis, I plan to perform cross-validation on the dataset by holding out 10% of the data to use as a test set and training my models on the other 90% of the data as a way to assess prediction accuracy. 

### Strengths and Limitations
|        There are several limitations to this study. For example, the VICE News dataset spans over the years 2010 to 2016, and neither of my models will be exploring time series data whatsoever. Thus, the aspect of time may be playing some sort of effect on my response variable that I am not accurately capturing in my models. Additionally, when cleaning the data, there were several "Unknown" values in the fatality column. Unknown values were assumed to represent "No" in this study, although this may not necessarily be true, to allow for more straightforward cleaning of the data and not lose too many observations in the data. Finally, there were less than 200 cases in which there were multiple victims in my final dataset, which is a relatively small sample size. 
