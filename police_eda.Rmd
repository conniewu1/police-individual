...
temp <- final_eda %>% 
  group_by(city, fatal) %>% 
  summarise(btw = black/white) %>% 
  unique

plot <- ggplot(temp, aes(x = btw, y = fatal, fill = fatal)) + 
    geom_boxplot()  + 
  labs(x = "Black to White Ratio", y = "Fatal vs. Non-Fatal Shooting", title = "City Black to White Ratio Distributions for Fatal vs. Non-Fatal Police Shootings") +
  theme(legend.text = element_text(size = 8),
        legend.key.size = unit(0.4, "cm"),
        axis.text.x = element_text(angle = 10, hjust = 1, size = 8),
        plot.title = element_text(size=10, hjust = 0.5))
...
```
Means are not that different, will probably just use percent black and percent white then.
